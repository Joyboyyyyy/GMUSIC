...
const ChangePasswordScreen = () => {
  ...
  const handleChangePassword = async () => {
    if (!currentPassword.trim()) return showAlert('Error', 'Please enter your current password', [{ text: 'OK', onPress: hideAlert, style: 'default' }], 'alert-circle-outline', 'error');
    if (!isPasswordValid) return showAlert('Error', 'Please ensure your new password meets all requirements', [{ text: 'OK', onPress: hideAlert, style: 'default' }], 'alert-circle-outline', 'error');
    if (!passwordsMatch) return showAlert('Error', 'New passwords do not match', [{ text: 'OK', onPress: hideAlert, style: 'default' }], 'alert-circle-outline', 'error');
    setLoading(true);
    try {
      await api.post('/api/auth/change-password', { currentPassword: currentPassword.trim(), newPassword: newPassword.trim() });
      showAlert('Success', 'Your password has been changed successfully.', [{ text: 'OK', onPress: () => { hideAlert(); navigation.goBack(); }, style: 'default' }], 'checkmark-circle', 'success');
    } catch (error: any) {
      console.error('[ChangePassword] Error:', error);
      showAlert('Error', error.response?.data?.message || 'Failed to change password.', [{ text: 'OK', onPress: hideAlert, style: 'default' }], 'alert-circle-outline', 'error');
