...
const FeedbackScreen = () => {
  ...
  const handleSubmit = async () => {
    if (!message.trim()) { Alert.alert('Error', 'Please enter your feedback message'); return; }
    setLoading(true);
    try {
      await api.post('/api/feedback', { type: selectedType, subject: subject.trim() || null, message: message.trim() });
      Alert.alert('Thank You!', 'Your feedback has been submitted successfully.', [{ text: 'OK', onPress: () => navigation.goBack() }]);
    } catch (error: any) {
