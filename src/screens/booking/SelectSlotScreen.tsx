...
const SelectSlotScreen = () => {
  ...
  useEffect(() => {
    // Set building ID from route params if not set
    const currentBuildingId = buildingId || storeBuildingId;
    if (currentBuildingId && !storeBuildingId) {
      useBookingStore.getState().setBuildingId(currentBuildingId);
    }
  ...
  const loadTimeSlots = async (buildingIdParam?: string) => {
    ...
    try {
      ...
      const response = await api.get(
        `/api/buildings/${currentBuildingId}/slots`,
        {
          params: {
            date: selectedDate,
          },
      ...
      const slots = response.data.map((slot: TimeSlot) => ({
        time: slot.time,
  ...
  const handleConfirmBooking = () => {
    ...
    if (status !== 'authenticated') {
      // Set redirect path to return to this screen after login
      const currentBuildingId = buildingId || storeBuildingId;
      if (!currentBuildingId) {
        Alert.alert('Error', 'Building ID is missing');
        return;
            ...
            {loading ? (
              ...
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptyText}>
                  No time slots are available for this date. Please try another date.
                </Text>
              </View>
            ) : timeSlots.length === 0 ? (
              <View style={styles.emptyContainer}>
          ...
          <View style={styles.footer}>
            ...
            >
              <Text style={styles.confirmButtonText}>
                {status === 'authenticated' ? 'Confirm Booking' : 'Login to Confirm'}
              </Text>
